...
fn extract_result(output: &ProgramIO) -> Result<AuthorizationResult, Box<dyn std::error::Error>> {
    // ONNX model outputs a tensor with 2 values: [approved_score, risk_score]
    // Extract from the ProgramIO output tensor

    let tensor_data = &output.output.inner;

    if tensor_data.len() < 2 {
        return Err("Output tensor too short, expected at least 2 elements".into());
    }

    // Convert i32 values to f32 scores (values are scaled by 1000 in the model)
    let approved_score = tensor_data[0] as f32 / 1000.0;
    let risk_score = tensor_data[1] as f32 / 1000.0;
